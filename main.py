...
class Moon:
    ...
    def load_image(self) -> None:
        ...
        # Apply Gaussian blur
        self._image_data = ndimage.gaussian_filter(image_data, sigma=self.blur_sigma)
    ...
    def _fit_circle(self, x: np.ndarray, y: np.ndarray) -> tuple[tuple[float, float], float]:
    ...
    def _calculate_center_and_radius(self, x_contour_cutoff: int = 1_750, threshold: float = 1.2e4) -> None:
        ...
        filtered = [(x, y) for x, y in zip(x_contour, y_contour) if x > x_contour_cutoff]
        if not filtered:
            self._center = (0, 0)
            self._radius = 0
            return
    ...
    # I need to add tildeness because the selenographic coordinates are not quite right
    # and because the moon is slightl rotating even tho is tidaly locked

    def selenographic2pixel(self, latitude: np.ndarray, longitude: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        '''
        Latitude = phi
        Longitude = lambda 
        '''
        ...
        x_pixel = self.radius * np.sin(longitude) * np.cos(latitude) + self.center[0]
        y_pixel = self.radius * np.sin(latitude) + self.center[1]

        return x_pixel, y_pixel

    def pixel2selenographic(self, x_pixel: np.ndarray, y_pixel: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
    
    
    def plot(self, show_contours: bool = False, contour_threshold: float = 1.2e4) -> None:
...
def main() -> None:
    moon = Moon(
        name="Moon",
        file_path="LUNA-C-1, 2025-01-11, 1x5L, SkyWatcher, (C), SBIG ST-8300 CCD Camera_average_stacked.fits",
        blur_sigma=1.0
    )
...
def plot_craters_on_moon(moon: Moon, crater_file: str) -> None:
    """Plot craters on an existing moon plot."""
    import pandas as pd
    
    crater_df = pd.read_csv(crater_file)
    real_lat, real_long = crater_df['real_Phi'], crater_df['real_lambda']
    
    # You'd need to adapt selenographic_to_pixel to use moon's center/radius
    # Or make it a method of Moon
    x_pix, y_pix = selenographic_to_pixel(real_lat, real_long)
    plt.scatter(x_pix, y_pix, s=0.5, c='blue')
